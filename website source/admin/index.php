...
<th>Action</th>
    ...
    if (isset($_POST['banacc']))
    {
		...
		wh_log("discordWebhookHere", "Admin `{$username}` has banned user `{$un}` for reason `{$reason}`", "RestoreCord Admin Logs");
    ...
    if (isset($_POST['unbanacc']))
    {
		...
		wh_log("discordWebhookHere", "Admin `{$username}` has unbanned user `{$un}`", "RestoreCord Admin Logs");
    ...
    {	
		...
		wh_log("discordWebhookHere", "Admin `{$username}` has updated user `{$un}` email to `{$email}`, role to `{$role}`, and 2FA status to `{$totp}`", "RestoreCord Admin Logs");
